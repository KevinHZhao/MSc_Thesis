...
Population stratification, on the other hand, occurs when when the population is composed of genetically distinct sub-populations, such as when there are groups descending from various ancestries or with various lifestyles based on culture \cite{Hell+17}.
This may lead to spurious associations \cite{CardPalm03}, for example, if one sub-population has a higher disease prevalence than another while simultaneously having a larger prevalence for the variant allele, whereas a second sub-population has lower prevalences, the case group would oversample individuals from the former sub-population, while the control group would oversample individuals from the latter sub-population, leading to an incorrect conclusion that the larger prevalence of the variant allele in the case group causes the disease.
...
With this study design, the Weinberg group developed a log-linear approach \cite{Wilc+98, Wein+98, WeinUmba05} to analyzing this type of data, which has been refined to allow researchers to test for child and maternal effects, alongside various other genetically plausible factors.

\section{The log-linear model} \label{sec:loglin}

In the log-linear model, we model the multinomial probabilities of a triad in the population having genotypes $(\MM, \FF, \CC)$ given the child is a case using the data from a case triad study.
We use the approximation of the multinomial distribution as a set of Poisson distributions, where, given $k$ independent poisson variables with means $\lambda_1, \ldots, \lambda_k$, the conditional distribution of the counts $Y_1, \ldots, Y_k$ given $\sum_{i=1}^k Y_i = Y_. = n$ will be
\begin{align*}
    \Pr(Y_1 = n_1, \ldots, Y_k = n_k \mid Y_. = n) &= \frac{\Pr(Y_1 = n_1, \ldots, Y_k = n_k)}{\Pr(Y_. = n)}, \\
...
\end{equation}
hence as a log-linear model, we can express this as
...
\end{equation}
The terms in this model are readily interpretable: $\Pr(\DD = 1 \mid \MM, \FF, \CC)$ is the probability of disease based on the trio's genotypes (whose expression is how we associate the variant allele with disease), $\Pr(\CC \mid \MM, \FF)$ is the Mendelian inheritance probabilities (see \cref{tab:Mendel}), and $\Pr(\MM, \FF)$ is the mating type probability for the mother and father.
The final term, $\alpha = \Pr(\DD = 1)$, is the prevalence of disease, which cannot be estimated from case-triad data, hence this term is uninformative.
...
\subsection{Expressing disease risk based on genotype}
...
$1$ & $(2,1,2)$ & $\MM, \FF$ & $1$ & $1$ & $\kappa p^3(1-p)R^2S^2V^2I_{\MM}/\alpha$ & $\kappa\mu_1R^2S^2V^2I_{\MM}/(2\alpha)$ & $\kappa\nu_1C_1R^2S^2V^2I_{\MM}/(2\alpha)$ \\
\multicolumn{8}{c}{$\vdots$}                                                                                       \\
$3$ & $(1,1,1)$ & $\MM$      & $1$ & $1$ & $\kappa p^2(1-p)^2RSVI_{\MM}/\alpha$     & $\kappa\nu_5RSVI_{\MM}/(4\alpha)$       & $\kappa\mu_3RSVI_{\MM}/(4\alpha)$          \\
$3$         & $(1,1,1)$         & $\FF$ & $1$ & $1$ & $\kappa p^2(1-p)^2RSV/\alpha$ & $\kappa\nu_5RSV/(4\alpha)$ & $\kappa\mu_3RSV/(4\alpha)$                    \\
\multicolumn{8}{c}{$\vdots$}                                                                                       \\
$4$         & $(0,1,0)$         &       & $1$ & $1$ & $\kappa p(1-p)^3/\alpha$      & $\kappa \nu_7/(2\alpha)$    & $\kappa\mu_4 \left(1 - C_4 \right)/(2\alpha)$ \\
\multicolumn{8}{c}{$\vdots$}                                                                                       \\
$2$ & $(2,0,1)$ & $\MM$      & $0$ & $1$ & $p^2(1-p)^2RS^2I_{\MM}/\alpha$    & $\kappa\nu_3RS^2I_{\MM}/\alpha$      & $\kappa\mu_2 C_2RS^2I_{\MM}/\alpha$     \\
\multicolumn{8}{c}{$\vdots$}                                                                                       \\
